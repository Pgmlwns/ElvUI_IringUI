...
function IR:OptionsCallback()
	...
	-- [에러 방지] DB 구조가 없을 경우 강제 생성 (독립성 유지)
	...
	-- [유틸리티] 버튼들 (기존에 있던 설치, 미디어 업데이트 등 복구)
	misc.util = ACH:Group("유틸리티", nil, 10, "inline")
	misc.util.args.install = ACH:Execute("설치 가이드 열기", "IringUI 설치 창을 다시 엽니다.", 1, function() IR:InterceptInstaller(); E:ToggleOptionsUI() end)
	...
	misc.util.args.layout = ACH:Execute("레이아웃 업데이트", "IringUI 전용 레이아웃을 다시 적용합니다.", 3, function() if IR.UpdateLayout then IR:UpdateLayout() end; UpdateRL() end)
end
